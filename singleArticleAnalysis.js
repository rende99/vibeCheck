...
async function getArticleInfo(searchQuery){
    ...
    */
    ...
    //alert(JSON.stringify(privateData));
    ...
    for(let k = 0; k < privateData.result.length; k++){
        ...
        for(let j = 0; j < privateData.result[k].articles.length; j++){
            ...
            //check if title is the same as the one we reviewed:d
            //alert(JSON.stringify(privateData.result[k].articles[j].title))
            if(privateData.result[k].articles[j].title == searchQuery){
                //alert(privateData.result[k].articles[j].title)
                //gender stuff:
                document.getElementById('a1Title').innerHTML = searchQuery;
    ...
    //gender
    $(`#a1AnalysisRange0`).val(nrmale == 0 ? 0 : Math.floor(tsmale/nrmale));
    $(`#a1AnalysisRange1`).val(nrfemale == 0 ? 0 : Math.floor(tsfemale/nrfemale));
    $(`#a1AnalysisRange2`).val(nrother == 0 ? 0 : Math.floor(tsother/nrother));

    //age
    $(`#a1AnalysisRange3`).val(nrBaby == 0 ? 0 : Math.floor(tsBaby/nrBaby));
    $(`#a1AnalysisRange4`).val(nrYoung == 0 ? 0 : Math.floor(tsYoung/nrYoung));
    $(`#a1AnalysisRange5`).val(nrMid == 0 ? 0 : Math.floor(tsMid/nrMid));
    $(`#a1AnalysisRange6`).val(nrOld == 0 ? 0 : Math.floor(tsOld/nrOld));
    $(`#a1AnalysisRange7`).val(nrElderly == 0 ? 0 : Math.floor(tsElderly/nrElderly));
    
    //location
    $(`#a1AnalysisRange8`).val(nrNE == 0 ? 0 : Math.floor(tsNE/nrNE));
    $(`#a1AnalysisRange9`).val(nrSouth == 0 ? 0 : Math.floor(tsSouth/nrSouth));
    $(`#a1AnalysisRange10`).val(nrMW == 0 ? 0 : Math.floor(tsMW/nrMW));
    $(`#a1AnalysisRange11`).val(nrW == 0 ? 0 : Math.floor(tsW/nrW));
    $(`#a1AnalysisRange12`).val(nrAbroad == 0 ? 0 : Math.floor(tsAbroad/nrAbroad));
    
    //education
    $(`#a1AnalysisRange13`).val(nrNHS == 0 ? 0 : Math.floor(tsNHS/nrNHS));
    $(`#a1AnalysisRange14`).val(nrHS == 0 ? 0 : Math.floor(tsHS/nrHS));
    $(`#a1AnalysisRange15`).val(nrSUN == 0 ? 0 : Math.floor(tsSUN/nrSUN));
    $(`#a1AnalysisRange16`).val(nrABD == 0 ? 0 : Math.floor(tsABD/nrABD));
    $(`#a1AnalysisRange17`).val(nrMD == 0 ? 0 : Math.floor(tsMD/nrMD));

    //race
    $(`#a1AnalysisRange18`).val(nrBLK == 0 ? 0 : Math.floor(tsBLK/nrBLK));
    $(`#a1AnalysisRange19`).val(nrAMI == 0 ? 0 : Math.floor(tsAMI/nrAMI));
    $(`#a1AnalysisRange20`).val(nrASI == 0 ? 0 : Math.floor(tsASI/nrASI));
    $(`#a1AnalysisRange21`).val(nrHAW == 0 ? 0 : Math.floor(tsHAW/nrHAW));
    $(`#a1AnalysisRange22`).val(nrWHI == 0 ? 0 : Math.floor(tsWHI/nrWHI));
    $(`#a1AnalysisRange23`).val(nrHIS == 0 ? 0 : Math.floor(tsHIS/nrHIS));
    
    //affiliation
    $(`#a1AnalysisRange24`).val(nrALT == 0 ? 0 : Math.floor(tsALT/nrALT));
    $(`#a1AnalysisRange25`).val(nrCON == 0 ? 0 : Math.floor(tsCON/nrCON));
    $(`#a1AnalysisRange26`).val(nrMOD == 0 ? 0 : Math.floor(tsMOD/nrMOD));
    $(`#a1AnalysisRange27`).val(nrLTR == 0 ? 0 : Math.floor(tsLTR/nrLTR));
    $(`#a1AnalysisRange28`).val(nrLIB == 0 ? 0 : Math.floor(tsLIB/nrLIB));
    $(`#a1AnalysisRange29`).val(nrSOC == 0 ? 0 : Math.floor(tsSOC/nrSOC));
